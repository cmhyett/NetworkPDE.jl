...
@testset "constructing node PDESystem" begin
    ...
    function instantiate(::Type{FluxNode}, comp::AbstractNetworkComponent; name)
        ...
        eqs = [ρ * (dx/dt) * sum(cross_sections) ~ q + sum([ (dx/dt)*cross_sections[i]*tmp_ρ[i] - sgns[i]*cross_sections[i]*tmp_ϕ[i] for i in 1:num_inc_edges])]
...
@testset "completing dynamic network" begin
    ...
    dt = minimum([get_metadata(net.edges[i], :dx) for i in 1:length(net.edges)])/sqrt(gradient(pressure_from_density, 1.0)[1])
...
#        Dx = Differential(x)
        ...
        function dϕ(ρ, ϕ, dx, dt, β, pressure_from_density)
            ...
            y = ϕ[2:end-1] .- (dt/dx) .* (pressure_from_density(ρ[2:end]) .- pressure_from_density(ρ[1:end-1])) .-
                b .* (ϕ[2:end-1].*abs.(ϕ[2:end-1]));
            return (Finv(y,b) .- ϕ[2:end-1])./dt
        end

        drho = dρ(ρ,ϕ,dx,dt);
        internal_eqs = vcat(Symbolics.scalarize.([Dt(ρ) ~ drho; Dt(ϕ[2:end-1]) ~ dϕ(ρ, ϕ, dx, dt, β, pressure_from_density)])...)
    ...
    function connect(net::Network; name)
        ...
        for v in net.vertices
            ...
            for i in 1:num_inc_edges
                ...
                if (sgns[i] == 1)
                    push!(edge_ρs, unknowns(model, model.var_to_name[:ρ][length(model.var_to_name[:ρ])]))
                    push!(edge_ϕs, unknowns(model, model.var_to_name[:ϕ][length(model.var_to_name[:ϕ])]))
                else
                    push!(edge_ρs, unknowns(model, model.var_to_name[:ρ][1]));
                    push!(edge_ϕs, unknowns(model, model.var_to_name[:ϕ][1]));
                end
            end
            push!(coupling_eqs, node_ρ * (1/dt) * sum([edge_dxs[i]*cross_sections[i] for i in 1:num_inc_edges]) ~ node_q + sum([ (edge_dxs[i]/dt)*cross_sections[i]*edge_ρs[i] - sgns[i]*cross_sections[i]*edge_ϕs[i] for i in 1:num_inc_edges]))
            for i in 1:num_inc_edges
                push!(coupling_eqs, Dt(edge_ϕs[i]) ~ sgns[i]*(pressure_from_density(node_ρ) - pressure_from_density(edge_ρs[i])))
            end
        end
    ...
    function instantiate(network::Network; name)
        ...
        for i in 1:length(network.edges)
            add_metadata(network.edges[i], :model,
                         instantiate(get_metadata(network.edges[i], :type), network.edges[i], name=Symbol("edge_$(i)")));
        end
        for i in 1:length(network.vertices)
            add_metadata(network.vertices[i], :model,
                         instantiate(get_metadata(network.vertices[i], :type), network.vertices[i], name=Symbol("node_$(i)")))
        end
