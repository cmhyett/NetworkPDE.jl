...
function get_params(g::MetaGraph)
    params = []
    for edge in edges(g)
        has_prop(g, edge, :params) ? push!(params, [get_prop(g, edge, :params)...]) :
        continue
    end
    for vertex in vertices(g)
        has_prop(g, vertex, :params) ? push!(params, [get_prop(g, vertex, :params)...]) :
        continue
    end
    return vcat(params...)
end
...
function create_system(g::AbstractGraph, t, dx, params)
    ...
    function dϕ_bdy(ρ, ϕ, dx, dt) #length(ρ)==2,length(ϕ)=1
        return -(dt / dx) * (p(ρ[2]) - p(ρ[1])) -
               (β * dt) * (ϕ[1] * abs(ϕ[1])) / (ρ[1] + ρ[2])
    end
