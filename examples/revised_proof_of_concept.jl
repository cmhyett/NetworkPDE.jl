...
function create_network()::Network
    e1_params = Dict(:L => 100.0, :λ => 0.11, :D => 0.25, :from_node = 1,
        :to_node = 2, :instantiate => edge_factory!)
    e2_params = Dict(:L => 50.0, :λ => 0.11, :D => 0.25, :from_node => 2,
        :to_node => 3, :instantiate => edge_factory!)
    e1 = Edge(1, e1_params)
    e2 = Edge(2, e2_params)

    v1_params = Dict(:is_flux => true, :flux => zeros(3), :incident_edges => [1],
        :instantiate => flux_node_factory!)
    v2_params = Dict(:is_flux => true, :flux => zeros(3), :incident_edges => [1, 2],
        :instantiate => flux_node_factory!)
    v3_params = Dict(:is_flux => true, :flux => zeros(3), :incident_edges => [2],
        :instantiate => flux_node_factory!)
